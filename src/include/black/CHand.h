#pragma once
// CHand — the player's hand: the object the HandState machine acts on.
//
// The original CHand is an ~18 KB struct shared with the renderer, the
// interface layer and the animation system, and the HandState bodies reach
// into it at raw offsets. We do not have those subsystems, so this is a host
// struct holding the fields the state machine actually reads, each annotated
// with the original offset it was recovered from (tools/decomp, vtable dump of
// the 12 HandState classes). It is deliberately NOT byte-compatible: filling
// 18 KB we cannot populate would be pretence, and nothing here is loaded from
...
struct HandDrawHooks {
    ...
    void (*draw_hand)(CHand* hand, uint32_t anim_id, uint32_t frame,
                      LHMatrix* transform) = nullptr;
    // Draw whatever object the hand is holding.
    void (*draw_held)(CHand* hand, GameThing* held) = nullptr;
